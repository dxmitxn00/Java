...
class Player{
   ...
   public int getPreIndex() {  
      return preIndex;
   // 접근 제어자가 private 여서 변수를 바로 호출할 수 없음
   // get메소드를 통해서 값을 반환해 가져와야함
   }
   public void setPreIndex(int preIndex) {
      this.preIndex = preIndex;
   // 접근 제어자가 private 여서 단순 연산으로 값을 변경할 수 없음
   // set 메소드를 통해서 입력받은 값을 기존의 변수에 대입해줌
   }
...
public class Answer02 {
   public static void main(String[] args) {
      ...
      player.addPokemon(new Metamon()); // 메소드 시그니처가 void addPokemon(Pokemon pokemon)임
      player.addPokemon(new Metamon()); // 인자를 Pokemon클래스의 객체로 받아야해서
      									// 인자값으로 생성자를 호출해 객체 배열에 값을 초기화해줌
